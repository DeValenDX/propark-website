...
export default function JobBoard() {
  ...
  const validateEmailsMatch = (email1: string, email2: string): string => {
    if (!email2) {
      return "Debes repetir tu correo electrónico";
    }
    if (email1 !== email2) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newErrors = {
      ...
      repetirCorreo: validateEmailsMatch(
        formData.correo,
        formData.repetirCorreo
      ),
      telefono: getPhoneErrorMessage(formData.telefono),
      cv: !formData.cv ? "Debes adjuntar tu CV" : "",
    };
    ...
    try {
      ...
      formDataToSend.append("delegacion", formData.delegacion);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col md:col-span-2">
                  <label
                    ...
                    className="mb-1 text-sm text-gray-700 font-medium"
                  >
                    Envíanos tu CV:<span className="text-red-500">*</span>
                  </label>
                  <input
                    id="cv"
                    name="cv"
                    type="file"
                    onChange={handleChange}
                    className={`w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#00d4ff] focus:outline-none bg-white ${
